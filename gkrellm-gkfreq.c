...
#define SPACING_BETWEEN_ROWS    0
#define SPACING_BETWEEN_COLS    5
...
static void create_plugin(GtkWidget *vbox, gint first_create) {
  ...
  gint w_avail = gkrellm_chart_width() - m->left - m->right;
  ...
  for (i=0; i<freqs2show; i++) {
    ...
    cpu[i].label_cpu = gkrellm_create_decal_text(panel,
                            "Ay", /* string used for vertical sizing */
                            ts_alt,
                            style,
    ...
    gint sw = gkrellm_gdk_string_width(ts->font, "XXXX MHz") + ts->effect;
    cpu[i].label_freq->x = w_avail + m->left - sw;
